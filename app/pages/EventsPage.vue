    ...
    <Page actionBarHidden="true">
        <GridLayout rows="auto, *" class="bg-background">
            ...
            <StackLayout v-if="events.length === 0" row="1" class="p-8" verticalAlignment="center" horizontalAlignment="center">
                <Label :text="$t('events.empty')" class="text-lg font-semibold text-foreground text-center mb-2" />
                <Label :text="$t('events.emptyHint')" class="text-sm text-muted-foreground text-center" textWrap="true" />
            </StackLayout>
                ...
                <template #default="{ item }">
                    <StackLayout class="px-4 pt-3">
                        <GridLayout rows="auto, auto, auto" columns="*, auto" class="card" androidElevation="2" @tap="openEvent(item)">
                            ...
                            <Label row="0" col="1" :text="statusLabel(item.status)" :class="statusBadgeClass(item.status)" verticalAlignment="top" />
                            ...
                            <Label row="2" col="1" :text="formatValue(item.totalValue)" class="text-base font-bold text-foreground mt-2" />
...
function formatValue(valor: number): string {
    return valor === 0 ? t('common.free') : 'R$ ' + valor.toLocaleString('pt-BR');
}

function statusLabel(status: string): string {
    switch (status) {
        case 'in_progress': return t('events.statusInProgress');
        case 'scheduled': return t('events.statusScheduled');
        case 'ended': return t('events.statusEnded');
