    ...
    <Page actionBarHidden="true">
        <GridLayout rows="*" columns="*">
            <ScrollView row="0" col="0" class="scroll-wrap">
                <StackLayout class="order-update-page bottom-pad">
                    <StackLayout class="header">
                        <GridLayout rows="auto" columns="*, auto" class="header-row">
                            ...
                            <StackLayout row="0" col="1" :class="'badge badge-' + orderStatus.toLowerCase()">
                                <Label :text="statusBadgeLabel" class="badge-text" />
                            </StackLayout>
                        </GridLayout>
                        <GridLayout rows="auto" columns="*, auto" class="header-client">
                            <StackLayout row="0" col="0">
                                <Label :text="clientName" class="header-client-name" />
                                <Label :text="clientDoc" class="header-client-doc" />
                            </StackLayout>
...
const orderId = ref('ORD-001');
const orderStatus = ref<'Open' | 'Completed'>('Open');
const observation = ref('Entregar no período da manhã');

const clientName = ref('Empresa Alpha Ltda');
const clientDoc = ref('12.345.678/0001-90');
const paymentMethodName = ref('Credit card');
const displayOrderValue = 'R$ 1.250,00';
const displayTotalItems = '5 itens';

const statusBadgeLabel = ref('Aberto');
